...
> Every function in the model grouped by its job, plus the data-flow chain. Source tags: **[Problem doc]** = from `ProblemHeliostat.pdf` Appendix; **[standard]** = general math/geometry, not in the doc. Current script: `heliostat_step6.py` (Steps 1–6 complete: all 5 optical losses + Table 1/2 reporting, built on `heliostat_step5_numba.py`).
...
FOR each of 60 timestamps:
   ...
   FOR each mirror i:
       ...
       η_trunc = monte_carlo_trunc(i, ŝ, t̂, N=200)          # D (Step 5, Monte-Carlo)
       η = η_cos · η_at · rho · η_sb · η_trunc               # 5 losses multiplied
       field_power += DNI · area · η                         # [Problem doc §3]
       accumulate η_cos, η_sb, η_at, η_trunc, η sums          # Step 6 (Table 1/2)

   store field_power, mean(η_cos), mean(η_sb), mean(η_at), mean(η_trunc), mean(η)
